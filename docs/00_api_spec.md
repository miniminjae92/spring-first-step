...
- GET, PUT: 멱등성 O
- PATCH: 특정 값 수정은 멱등성 O, 증가시키는 연산같은 경우 멱등성 X
- DELETE: 멱등성 O, 응답 상태코드는 다를 수 있다.
    <details>
    <summary>상태 코드가 왜 중요한가요? (클릭)</summary>

  HTTP 상태 코드는 서버와 클라이언트 사이의 **최소한의 약속**입니다.  
   본문을 읽기 전에 응답의 성격을 즉시 파악하게 함으로써 시스템의 효율성을 극대화합니다.
  1. **인프라 관점**: 로드 밸런서는 본문 파싱 없이 코드만 보고 라우팅 결정
  2. **라이브러리 관점**: 에러 핸들링 로직(catch) 자동화 가능
  3. **표준화**: 전 세계 개발자의 공용어
  </details>
...
- 성공(204): 본문(Body)을 사용하지 않습니다.
- 실패(404): 공통 메시지 응답 규격을 사용합니다.
...
- 성공(204): 본문(Body)을 사용하지 않습니다.
- 실패(404, 400): 공통 메시지 응답 규격을 사용합니다.
...
| **상태 코드** | **이름**    | **발생 상황**                                                  |
| ------------- | ----------- | -------------------------------------------------------------- |
| **200**       | OK          | 게시물 조회(GET) 요청이 성공하고 데이터가 반환됨               |
| **201**       | Created     | 새로운 게시물이 성공적으로 생성됨 (Location 헤더 포함)         |
| **204**       | No Content  | 게시물 수정(PUT) 또는 삭제(DELETE)가 성공하여 본문 없이 응답함 |
